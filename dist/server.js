...
async function getDataForDateAndCity(selectedDate, cityName) {
...
async function getPreviousDayData(selectedDate, cityName) {
...
async function analyzeWeekdayTrend(cityName, allData) {
...
function getLocalHolidayMultiplier(cityName, month, day) {
    ...
    let countryPrefix = '';
    if (isFrance)
        countryPrefix = 'FRANCE_';
    else if (isNorway)
        countryPrefix = 'NORWAY_';
    else if (isKorea)
        countryPrefix = 'KOREA_';
...
async function calculateTargetRevenue(selectedDate, cityName) {
    ...
    if (cityName.toLowerCase().includes('cannes'))
        locationMult = 1.15;
    else if (cityName.toLowerCase().includes('nice'))
        locationMult = 1.05;
    else if (cityName.toLowerCase().includes('seoul'))
        locationMult = 1.25;
    else if (cityName.toLowerCase().includes('oslo'))
        locationMult = 1.1;
